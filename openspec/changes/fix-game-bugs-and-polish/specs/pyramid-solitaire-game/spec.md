...
Cards with hearts (♥) or diamonds (♦) suits SHALL be rendered with red text. Cards with spades (♠) or clubs (♣) SHALL use the default light text color.

#### Scenario: Heart and diamond cards display in red
- **WHEN** a card with suit ♥ or ♦ is rendered on the board or in the discard area
- **THEN** the card text SHALL be rendered in a red color

#### Scenario: Spade and club cards display in default color
- **WHEN** a card with suit ♠ or ♣ is rendered
- **THEN** the card text SHALL use the default foreground color

### Requirement: Removed cards do not occupy layout space
Cards that have been removed from the pyramid SHALL NOT reserve visual space in the row layout. Sibling cards in the same row SHALL fill the available space.
